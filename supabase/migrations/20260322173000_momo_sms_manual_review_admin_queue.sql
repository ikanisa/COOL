...
comment on function public.get_momo_sms_manual_review_queue(integer, integer) is
  'Admin-only queue for generic M-Money SMS manual reviews that are not scoped to the bank allocation workspace.';

revoke all on function public.get_momo_sms_manual_review_queue(integer, integer) from public;
grant execute on function public.get_momo_sms_manual_review_queue(integer, integer)
...
begin
  ...
  v_note := coalesce(
    nullif(btrim(p_note), ''),
    'Admin closed this SMS review because it does not map to an app payment target.'
  );
  ...
  set
    ...
    match_type = 'admin_manual_rejection',
    match_status = 'rejected',
    notes = v_note,
    metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      'manual_rejection', true,
      'manual_rejection_actor_id', auth.uid(),
      'manual_rejection_source', 'admin_operations',
...
comment on function public.admin_reject_momo_sms_manual_review(uuid, text) is
  'Admin-only operation that closes a generic M-Money SMS manual review as not app-linked without deleting the underlying wallet history.';

revoke all on function public.admin_reject_momo_sms_manual_review(uuid, text) from public;
grant execute on function public.admin_reject_momo_sms_manual_review(uuid, text)
  to authenticated, service_role;
...
comment on function public.admin_reject_momo_sms_manual_review_batch(uuid[], text) is
  'Admin-only batch operation that closes generic M-Money SMS manual reviews as not app-linked.';

revoke all on function public.admin_reject_momo_sms_manual_review_batch(uuid[], text) from public;
