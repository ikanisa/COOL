-- ==========================================================================
-- Fix: Remove references to non-existent partner_users table
-- The partner_users table was never created. These RPCs incorrectly
-- reference it for custodian access checks. Fix: use direct is_admin()
-- + owner_user_id check instead.
-- ==========================================================================

-- ── 1. Fix get_momo_sms_manual_review_queue ─────────────────────────────
...
-- ── 2. Fix admin_reject_momo_sms_manual_review ──────────────────────────
...
begin
  ...
  -- Authorization: must be admin or owner of the receiver account
  if not public.is_admin() then
    if v_pra.id is null then
      raise exception 'Admin privileges required to reject unowned reviews.';
    end if;
    if v_pra.owner_user_id <> auth.uid() then
      raise exception 'Admin or account owner privileges required.';
    end if;
  end if;
...
-- ── 3. Fix admin_reject_momo_sms_manual_review_batch ────────────────────
