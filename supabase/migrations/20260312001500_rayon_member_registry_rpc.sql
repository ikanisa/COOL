-- ==========================================================================
-- Rayon Sports member registry RPC
-- ==========================================================================
-- Exposes a safe, searchable member registry to authenticated clients without
-- broadening direct access to public.users beyond existing RLS policies.
-- Search and filtering happen server-side before pagination.
-- ==========================================================================
...
begin
  ...
  select
    membership.user_id,
    coalesce(nullif(btrim(profile.full_name), ''), 'Rayon Fan') as display_name,
  ...
  where membership.partner_id = p_partner_id
    ...
    and (
      v_search is null
      or coalesce(nullif(btrim(profile.full_name), ''), 'Rayon Fan')
          ilike '%' || v_search || '%'
