...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requestHash = cleanString(payload.request_hash, "request_hash", 64).toLowerCase();
    const token = cleanString(payload.integrity_token, "integrity_token", 20_000);
    ...
    if (group.group_is_public !== false ||
        group.receiver_momo_number_hash !== receiverHash) {
      return jsonResponse({ error: "Only private Android groups are allowed" }, 400);
    }
    const groupRequest = {
    ...
    const requestDetails = decoded.requestDetails as Record<string, unknown> | undefined;
    const appIntegrity = decoded.appIntegrity as Record<string, unknown> | undefined;
    const deviceIntegrity = decoded.deviceIntegrity as Record<string, unknown> | undefined;
    ...
    if (verdictPackage !== packageName || appVerdict !== "PLAY_RECOGNIZED" ||
    ...
    const { data: capability, error } = await serviceClient().rpc(
      "mint_native_action_capability",
      {
        ...
        capability_action: "group.create",
    ...
    return jsonResponse({
      ...
      capability_expires_at: new Date(timestampMillis + maxTokenAgeMs).toISOString(),
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Play Integrity failed";
