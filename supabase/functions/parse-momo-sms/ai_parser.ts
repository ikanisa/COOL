...
function tryParseDebitPayment(
  ...
  const payeeName = merchantCode
    ? null
    : cleanupCounterpartyName(payeeSegment);
...
function tryParseInboundTransfer(
  ...
  const parsed: ParsedSms = {
    ...
    ai_summary: `Received ${formatMoney(amount)} RWF from ${payerName ?? "sender"}.`,
    recurring_pattern_hint: "unknown",
    narrative: body,
...
function cleanupCounterpartyName(value: string | null | undefined): string | null {
  const trimmed = value?.trim();
  if (!trimmed) {
    return null;
  }
...
function extractLastThreeDigits(value: string | null | undefined): string | null {
  if (!value) {
    return null;
  }
  const digits = value.replaceAll(/\D/g, "");
