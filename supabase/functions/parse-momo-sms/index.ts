...
function ledgerEntryType(parsed: ParsedSms): "credit" | "debit" {
...
function looksLikeRayonTicketReference(reference: string): boolean {
  return reference.trim().toUpperCase().startsWith("RS-TICKET-");
}

function looksLikeRayonShopReference(reference: string): boolean {
  return reference.trim().toUpperCase().startsWith("RS-SHOP-");
}

function looksLikeRayonSupportReference(reference: string): boolean {
...
async function reconcileParsedSms(
  adminClient: ReturnType<typeof createAdminClient>,
  rawSms: RawSmsRecord,
  parsed: ParsedSms,
  parsedSmsId: string,
