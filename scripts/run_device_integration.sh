...
seed_sms_suite() {
  ...
  DEVICE="$DEVICE" bash "$ROOT_DIR/scripts/seed_android_momo_sms_test_inbox.sh" clean >/dev/null 2>&1 || true
...
capture_failure_diagnostics() {
  ...
  if command -v adb >/dev/null 2>&1; then
    ...
    if target_requires_sms_permission "$target"; then
      DEVICE="$DEVICE" bash "$ROOT_DIR/scripts/seed_android_momo_sms_test_inbox.sh" query \
