...
bash "$ROOT_DIR/scripts/verify_release_metadata.sh"
...
deno run \
  ...
  "$ROOT_DIR/scripts/verify_firebase_app_check.ts"

echo "==> release readiness"
bash "$ROOT_DIR/scripts/release_readiness.sh"

echo "==> signed Android release APK"
bash "$ROOT_DIR/scripts/build_production.sh"

echo "==> signed Android release AAB"
bash "$ROOT_DIR/scripts/build_play_release.sh"
