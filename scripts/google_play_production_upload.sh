...
RELEASE_NAME="${RELEASE_NAME:-Collect 1.2.2 (9)}"
...
Environment:
  ...
  OUTPUT_PATH                          default: .cache/google_play_optimization/android_publisher_upload_v9.json
...
if [[ -z "$OUTPUT_PATH" ]]; then
  if [[ "$SUBMIT" == "true" ]]; then
    OUTPUT_PATH=".cache/google_play_optimization/android_publisher_upload_v9.json"
  else
    OUTPUT_PATH=".cache/google_play_optimization/android_publisher_upload_v9_dry_run.json"
  fi
fi
