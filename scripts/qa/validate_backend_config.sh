...
production_only_release=0
if [[ "${COOL_PRODUCTION_ONLY_RELEASE:-0}" == "1" ]]; then
  production_only_release=1
fi

skip_staging_backend_validation=0
if [[ "$production_only_release" == "1" || \
      "${COOL_SKIP_STAGING_BACKEND_VALIDATION:-0}" == "1" ]]; then
...
elif [[ "$production_only_release" == "1" ]]; then
  echo "==> staging backend: skipped (COOL_PRODUCTION_ONLY_RELEASE=1)"
