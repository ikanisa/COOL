...
async function getAccessToken(
  ...
  const response = await fetch(tokenUrl, {
    ...
    body: `grant_type=${encodeURIComponent(grantType)}&assertion=${
  ...
  if (!response.ok) {
    throw new Error(
      `Firebase OAuth token exchange failed: ${response.status} ${await response
        .text()}`,
    );
  }
...
async function fetchProviderConfig(
  ...
  if (!response.ok) {
    throw new Error(
      `App Check config fetch failed for ${name}: ${response.status} ${await response
        .text()}`,
    );
  }
...
export async function main() {
  ...
  const androidConfig = await fetchProviderConfig(
    accessToken,
    androidConfigName,
  );
