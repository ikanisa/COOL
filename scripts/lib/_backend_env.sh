...
require_distinct_staging_and_production_supabase_projects() {
  ...
  local production_only="${COOL_PRODUCTION_ONLY_RELEASE:-0}"
  if [[ "$allow_shared" == "1" || "$production_only" == "1" ]]; then
    if [[ "$production_only" == "1" ]]; then
      echo "==> production-only release mode: staging/project separation check skipped"
    fi
    return 0
  fi
