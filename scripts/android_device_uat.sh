...
APPLICATION_BINARY="${ANDROID_UAT_APPLICATION_BINARY:-}"
...
if [[ -f "$DRIVER" ]]; then
  ...
  runner="drive"
  if [[ -n "$APPLICATION_BINARY" ]]; then
    [[ "$MOBILE_EVIDENCE_MODE" == "true" && "$FLAVOR" == "dev" ]] || \
      fail "A retained application binary is supported only for dev fixture QA."
    [[ -f "$APPLICATION_BINARY" ]] || fail "Retained application binary is missing."
    # The existing variant marker, case count and screenshot checks still
    # validate the compiled fixture. This avoids another build on a busy host.
    cmd+=(--use-application-binary="$APPLICATION_BINARY")
  fi
else
  cmd=(
    "$FLUTTER"
