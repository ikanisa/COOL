    ...
    if (status.tier != FanTier.platinum && status.pointsToNextTier <= 50) {
      ...
      quests.add(
        CoolQuest(
          ...
          subtitle:
              '${status.pointsToNextTier} points to unlock $nextTier tier',
          route: '/profile',
          priority: 90,
        ),
      );
    ...
    if (pendingMomoCount > 0) {
      quests.add(
        CoolQuest(
          ...
          subtitle:
              '$pendingMomoCount pending MoMo confirmation${pendingMomoCount > 1 ? "s" : ""}',
          route: '/momo',
          priority: 80,
        ),
      );
    ...
    if (status.currentStreak > 0 && status.streakGraceRemaining == 0) {
      quests.add(
        CoolQuest(
          ...
          subtitle:
              'Do an action today to keep your ${status.currentStreak}-day streak',
          route: '/home',
          priority: 95,
        ),
      );
