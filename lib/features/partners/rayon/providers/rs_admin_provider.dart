...
final rsAdminMatchesProvider =
    FutureProvider.autoDispose<List<RsMatch>>((ref) async {
...
final rsAdminProductsProvider =
    FutureProvider.autoDispose<List<RsProduct>>((ref) async {
      final repo = ref.watch(rayonSportsRepositoryProvider);
      return repo.adminGetAllProducts();
    });

/// All shop orders (across all users) for admin listing.
final rsAdminOrdersProvider =
    FutureProvider.autoDispose<List<RsShopOrder>>((ref) async {
      final repo = ref.watch(rayonSportsRepositoryProvider);
      return repo.adminGetAllOrders();
    });

/// All tickets (optionally filtered by match) for admin listing.
final rsAdminTicketsProvider =
    FutureProvider.autoDispose.family<List<RsTicket>, String?>((
      ref,
      matchId,
    ) async {
      final repo = ref.watch(rayonSportsRepositoryProvider);
...
final rsAdminMembersProvider =
    FutureProvider.autoDispose<List<FanMembership>>((ref) async {
      final repo = ref.watch(rayonSportsRepositoryProvider);
      return repo.adminGetAllMembers();
    });

/// All fan clubs for admin listing.
final rsAdminFanClubsProvider =
    FutureProvider.autoDispose<List<RsFanClub>>((ref) async {
      final repo = ref.watch(rayonSportsRepositoryProvider);
...
final rsAdminMatchTicketStatsProvider =
