...
class _FakeAdminSavingsRepository extends AdminSavingsRepository {
  ...
  Future<Map<String, dynamic>> bulkAddGroupMembers({
    ...
    bulkAddCalls.add(<String, dynamic>{
      'groupId': groupId,
      'members': members,
    });
...
void main() {
  ...
  testWidgets('adds a member through the bulk-add contract', (tester) async {
    ...
    expect(
      repository.bulkAddCalls.single['members'],
      <Map<String, String>>[
        <String, String>{
          'phone': '+250788111333',
          'display_name': 'Jeanne',
  ...
  testWidgets('allocates a manual contribution to the selected member', (
    tester,
  ) async {
    _configureTallViewport(tester);
    final repository = _FakeAdminSavingsRepository();
  ...
  testWidgets('closes the group through the update contract', (tester) async {
    _configureTallViewport(tester);
    final repository = _FakeAdminSavingsRepository();
    await _pumpScreen(tester, repository);
