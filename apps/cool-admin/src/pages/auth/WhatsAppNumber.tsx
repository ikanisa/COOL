...
import { sendAdminWhatsAppOtp } from "@/lib/whatsapp-otp";
...
export function WhatsAppNumber() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const phone = await sendAdminWhatsAppOtp(number);
    ...
    <div className="bg-surface-low/60 backdrop-blur-[40px] rounded-[3rem] p-10 shadow-[0_20px_80px_-10px_rgba(13,10,39,0.8),inset_1px_1px_2px_rgba(255,255,255,0.05),inset_-1px_-1px_2px_rgba(0,0,0,0.4)] relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-3">
          <label htmlFor="phone-input" className="text-sm font-label font-medium text-primary/80 uppercase tracking-wider">WhatsApp Number</label>
        ...
        <Button 
          type="submit"
          disabled={number.length < 8 || loading}
