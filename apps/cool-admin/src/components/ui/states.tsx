...
export function TableSkeleton({ rows = 5, cols = 6 }: { rows?: number; cols?: number }) {
  // Deterministic widths to avoid layout shift on re-render
  const COL_WIDTHS = [80, 120, 100, 60, 90, 110, 70, 130, 85, 95];
        ...
        <div key={r} className="flex items-center gap-4 px-4 py-3 border-b border-zinc-100">
          {Array.from({ length: cols }).map((_, c) => (
            <div
              ...
              style={{ width: `${COL_WIDTHS[c % COL_WIDTHS.length]}px` }}
            />
          ))}
        </div>
      ))}
