...
| Play Integrity/security | Console showed Play App Signing and automatic protection enabled. `docs/release/GOOGLE_PLAY_OPERATIONAL_READINESS_2026-06-21.md` records a server-verified rollout plan and explicitly avoids a client-only false-positive implementation. | Design recorded, implementation pending | Implement server-verified Play Integrity for abuse-prone actions after Google Cloud project/API/service-account access is available. |
