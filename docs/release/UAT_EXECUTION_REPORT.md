...
Status: **AUTOMATED LOCAL, DEVICE SMOKE, AND ADMIN LIVE GATES PASS; LINKED
SENDER-HASH MIGRATION AND HUMAN RELEASE SIGNOFF PENDING**

Public launch remains **NO-GO** until real Android SMS access UAT is captured,
the linked sender-hash migration is applied, Android signing/iOS scope evidence
is recorded, and stakeholder/release-owner signoff is recorded.
...
| --- | --- | --- | --- | --- | --- |
| UAT-01 | Contributor | Open group, enter amount, create payment intent, launch MoMo USSD, and wait for SMS allocation. | Local repository/widget tests pass; linked rollback UAT fails until sender-hash migration is applied. | Backend blocked | Apply linked migration, then run live tester flow with real Android SMS evidence. |
...
| UAT-05 | Android SMS device | Grant Android SMS access and allow automatic SMS upload. | Edge/auth contracts pass; linked rollback UAT is blocked on sender-hash migration. | Pending | Apply linked migration and run physical Android SMS access UAT. |
...
| UAT-10 | Edge-case user | Invalid amount, expired intent, ambiguous amount, missing receiver authorization, failed auth. | Unit/contract tests and Edge auth UAT pass; linked rollback UAT is blocked on sender-hash migration. | Backend blocked | Apply linked migration and capture physical Android evidence. |
...
3. Run physical Android SMS access UAT with sanitized evidence.
4. Record Android signing review and iOS release scope evidence.
5. Refresh release evidence and release-owner signoff from current commands
   only.
