...
| P0-005 | Android release artifacts | Current Android release APK/AAB artifacts are older than the refactored Android/mobile sources. | `scripts/flutter_mobile_release_gate.sh --json` reports blocker key `android_release_artifacts`; `scripts/release_artifact_manifest.sh --json` blocks on stale APK/AAB artifacts. | Rebuild production Android APK/AAB from the current sources, then rerun mobile release and artifact-manifest gates. |

## P1 Items

| ID | Area | Finding | Evidence | Required action |
| --- | --- | --- | --- | --- |
| P1-001 | Release evidence | Release evidence must be regenerated after current Android artifacts, linked Supabase UAT, Android SMS UAT, and Admin PWA live proof are complete. | Current docs were refreshed to the SMS-first product definition; release gates remain NO-GO on current evidence gaps. | Regenerate UAT/release packet after all validators pass. |
...
- `scripts/flutter_mobile_release_gate.sh --json`: blocked on stale Android
  release APK/AAB artifacts, release signing review, and iOS release scope.
- `scripts/release_artifact_manifest.sh --json`: blocked because the existing
  Android release APK/AAB artifacts are older than current mobile sources.
