...
contributions. Members create payment intents in the app, complete payment
through MoMo off app, and contribution evidence is created when official MoMo
SMS is ingested, parsed, allocated, and posted to the ledger.
...
3. Android group creator creates a group; the receiver MoMo defaults from the
   creator profile and can be edited where the product permits.
...
7. Official MoMo SMS is uploaded, parsed, allocated to the pending intent, and
   posted to the immutable ledger.
