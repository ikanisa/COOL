...
- `PAYMENT_PROVIDER_FINALITY_SECRET_CURRENT` (server-only HMAC key, at least
  32 characters; never exposed to Flutter, native Android, browser code, logs,
  or evidence bundles)
- `PAYMENT_PROVIDER_FINALITY_SECRET_PREVIOUS` (optional server-only rotation
  key; remove after the bounded rollover window)
