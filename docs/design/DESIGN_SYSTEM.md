...
- Group names always render in one line with ellipsis on cards, media overlays, lists, and headers unless a full-detail edit field is active.
- Icon-first metadata is required for support, supporters, members, amount, group type, public/private, QR, receiver, owner, and visibility signals. Use `CollectSemanticIcons` and `docs/design/collect_runtime_tokens/collect_semantic_icon_keywords_2026-06-30.json` before adding visible labels.
