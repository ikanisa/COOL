...
**Deliverable status: 40 concepts and generation prompts prepared. Images generated: 0. Runtime integration: not implemented.** This document plans the image bank and its use in group creation; it does not close MOBILE-DESIGN-100 or replace DESIGN.md.
...
This is a representative first bank of **40 new photographs**, including object-led photographic still lifes. The five existing selectable Rwanda photos and the marketing portrait are not counted toward the 40. The bank is extensible; it is not a claim to exhaust all Rwandan customs, religious traditions or ways of contributing.

## What the current Collect implementation establishes

These observations come from the working tree reviewed on 6 September, not from the gallery screenshot alone.
...
| CollectionType in collect_models.dart | The five storage types are ikimina, sport, church, wedding and other. The configurable catalogue contains existing subtypes. | The eight image families and 40 image themes are a richer photo taxonomy, not replacement database group types. |
...
The FinScope 2024 indexed savings discussion identifies savings groups as a major informal savings channel. The plan gives savings and livelihoods eight images, while keeping informal groups distinct from licensed SACCOs. These are editorial coverage choices, not proportional population quotas. [NISR FinScope 2024](https://www.statistics.gov.rw/sites/default/files/documents/2024-09/Rwanda-Finscope-2024-Report_compressed.pdf), [RCA background on SACCOs](https://www.rca.gov.rw/cooperatives/about-saccos).
...
The three NISR/RCHA PDF references were available as search-index excerpts; full direct fetches timed out or returned 403. The access limitations and claim boundaries for all ten sources are recorded in [SOURCE-REVIEW.md](production/SOURCE-REVIEW.md). Source photographs, real beneficiaries, named organisations and quotes are not generation inputs.

## The 40-image production list
...
Coverage totals: savings/livelihoods 8; weddings 6; faith/giving 6; family/solidarity 6; education/skills 4; community projects 4; sport/culture 4; diaspora 2 = **40**.
...
The catalogue's eight families are browsing shortcuts. Its 40 theme keys identify precise intents such as traditional_wedding, moto_goal, school_supplies or muslim_giving. They do not change CollectionType or the stored purpose of a group.
...
| --- | --- |
| assets/group_covers/rwanda/catalog.v1.json | Production authority for the 40 concepts, draft labels, prompts and planned paths |
...
| --- | --- | --- |
| 1. Prepare | Finalise the 40 briefs, metadata, draft localisations and this integration plan | Catalogue validation and exported 40-job pack; completed in this task |
...
| 4. Prepare runtime assets | Review local plausibility/dress/objects; compress derivatives; refine focal points and labels | Forty reviewed covers/thumbs, source/derivative provenance, measured sizes and no missing files |
...
The first eight are visual calibration anchors, not eight images that have already been generated. The job list contains all 40; normal generation uses one built-in call per distinct image. It requires no API key. Review each batch promptly, correct a failed scene and proceed within the authorised production task; the batch structure is not a request for a new approval after every eight calls. A future production run should record actual elapsed time and regeneration count rather than promise a fixed completion time now.
...
- [All 40 standalone prompts](production/PROMPTBOOK.md)
- [40 built-in generation jobs](production/imagegen-jobs.jsonl)
...
**Current handoff:** the plan, scripts and all 40 prompts are saved. Generation, cultural/crop review, durable UI integration and native acceptance are the next production phases.
