...
| ID | Priority | Remaining task | Dependency or blocker | Required exit evidence | Owner | Status |
|---|---|---|---|---|---|---|
| RT-020 | P1 | Run VoiceOver traversal on iOS for authentication, five-tab shell, group, contribution, ledger, settings, and error states. | E-037 statically verifies stable labels and semantic tap actions. E-073 compiles/runs current source on an iOS Simulator, but Simulator cannot supply actual VoiceOver. E-075 proves the development-signing/UAT path. E-081 finds the exact physical iPhone paired and supported, but it remained locked through the bounded preflight; no runner or VoiceOver traversal started. Distribution provisioning remains separately open under RT-043. | Reading-order, label, state, action, and focus evidence with defects closed on the exact unlocked physical iPhone. | QA / device owner | External physical-iPhone accessibility gate; development UAT path is proven, actual VoiceOver evidence remains absent |
...
| RT-034 | P2 | Run iOS physical-device UAT. | E-075 proves the exact iPhone 12 Pro development-signing/install/launch path and a 35/35 Dark route matrix. E-077 adds guarded physical lifecycle and Camera Settings targets. E-081 again passed the staging prebuild on the paired device, but the phone remained locked through the 60-second preflight; the runner never started and no install, reset, route, screenshot, lifecycle, Camera, or VoiceOver evidence was accepted. | Keep the paired phone unlocked and awake, preferably over USB, then obtain accepted lifecycle state-preservation markers, native Camera denial/App-Settings enable/return recovery, and actual VoiceOver traversal. Physical-host screenshots are unavailable and must not be fabricated. | QA / Release owner / device owner | Blocked on an unlocked physical-device window; prior Dark routes and current Simulator Camera states pass, while current physical lifecycle, Camera recovery, and VoiceOver remain open |
...
| RT-037 | P2 | Close the remaining legacy Kotlin Gradle Plugin warnings. | E-074 corrects prior scanner false positives and proves all 14 resolved Android plugins future-source-ready. On 2026-08-05, the governed installation is Flutter 3.44.4 and the official stable release family remains 3.44; the required Flutter 3.47+ built-in-Kotlin gate is therefore not yet executable. | Controlled Flutter 3.47+ upgrade, `android.builtInKotlin=true`, clean compatibility gate, and complete Android build/regression matrix. | Flutter / upstream owners | Version-gated; source preparation is complete and platform enablement awaits a governed Flutter 3.47+ release |
...
| RT-042 | External | Prepare and approve store metadata, privacy labels/data safety, screenshots, support URLs, account-deletion disclosures, and live Play Console surfaces. | The local packet now includes the hash-pinned official icon, a 1024x500 deterministic derivative of Collect-owned brand art, two sanitized 1080x2160 current Android captures, current listing text, policy URLs, deletion disclosures, Data safety prompts, App Links, and version `1.2.2+10`. Google credentials are unavailable. | `scripts/google_play_console_audit_packet.sh --json` passes. The optimization gate is blocked only by Play Developer Reporting authentication and live Console surface audit. | Codex implementation owner / account holder | Local submission pack complete; authenticated Reporting/Console review and store acceptance remain external |
