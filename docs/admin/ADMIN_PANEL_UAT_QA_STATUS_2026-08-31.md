...
| --- | --- | --- |
| Exact release source | Pass | Commit `116d2819` was isolated from unrelated in-progress payment-rail work. Static analysis passed and all 288 Flutter tests passed. |
...
| Admin deployment | Pass | Cloudflare version `2c458f3c-0f33-47ae-898a-13d4acf096cb` serves 100% of `collect-admin`; version `f3b75543-a8cb-4257-98a2-a7b417021a59` is retained for rollback. The custom-domain header/asset gate and the production PWA runtime/cache probe pass. |
...
| Logout hardening | Deployed; fresh-session readback pending | UAT found one immediate protected-route re-entry after logout. The shell now invalidates the auth stream, guard and Admin identity after the awaited Supabase sign-out. Static analysis, the focused auth test, all 288 tests, build gates and live PWA gates pass on the deployed fix. A new authorized OTP cycle is required to verify the fresh live session and final logout redirect. |
