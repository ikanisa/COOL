...
| --- | --- | --- |
| **Maximum font weight** | `w800` (Bold) | w800 provides full authority with optimal stroke clarity on mobile OLED screens. **No w800 or w900 anywhere in UI code.** |
...
| --- | --- | --- | --- |
| **Hero** | `w800` (Bold) | `black`, `extraBold`, `bold` | Display titles, CTAs, hero amounts, key actions |
| **Emphasis** | `w700` (SemiBold) | `semibold` | Section headings, card titles, action labels |
| **Body** | `w500` (Regular) | `regular` | Labels, metadata, descriptions, utility text |

*Exceptions:* PDF export code (using `pw.FontWeight`) and emoji-only text spans are exempt from these constraints.

*   **Display & Headline (Space Grotesk):** These must be used at "maximum impact." Do not be afraid of `display-lg` (3.5rem). Use tight letter-spacing (-0.04em) and w800 Bold to create a "condensed monolith" effect. Headlines should feel like they are "stamped" into the clay surfaces.
...
| Role | Font | Access |
| --- | --- | --- |
| Display / Headline | Space Grotesk w700–w700 | `context.coolText.headline(...)` |
| Title / Body | Manrope w500–w700 | `context.coolText.display(...)` |
| Label / Utility | Inter w500–w700 | Theme default |
| Numeric / Mono | DM Mono w700–w700 | `context.coolText.mono(...)` |
...
*   **DON'T** use `FontWeight.w800` or `FontWeight.w900`. Maximum allowed weight is `w800` (Bold).
